    ...
    if check_status_clicked:
        try:
            response = requests.get(
                f"{API_BASE_URL}/health", auth=HTTPBasicAuth(USERNAME, PASSWORD)
            )
            ...
            if response.status_code == 200:
                ...
                st.sidebar.error(
                    f"Server is not available. Status code: {response.status_code}."
                )
...
def get_models() -> pd.DataFrame:
    ...
    - Если модели есть, преобразует полученные данные в pd.DataFrame и отдает ео
    - Если сервер выдает ошибку, выводит соответствующее сообщение
    """

    response = requests.get(
        f"{API_BASE_URL}/models", auth=HTTPBasicAuth(USERNAME, PASSWORD)
    )
    ...
    if response.status_code == 200:
        ...
        models_df.created_at = (
            pd.to_datetime(models_df.created_at) + dt.timedelta(hours=3)
        ).dt.strftime("(MSK) %Y-%m-%d %H:%M:%S")
        models_df.columns = [
            "Model ID",
            "Model Name",
            "Model Type",
...
def view_models() -> list:
    ...
    - Формирует интерактивную таблицу просмотра метаданных об��ченных моделей
    ...
    gb = GridOptionsBuilder.from_dataframe(
        models_df[["Model Name", "Model Type", "Created At", "Model ID"]]
    )
...
def load_csv() -> pd.DataFrame:
    ...
    - Позволяет сохранить загруженный датафрейм в MinIO
    """

    csv_file = st.file_uploader("""Choose a `.csv` file to upload:""", type="csv")
    ...
    if csv_file is not None:
        try:
            ...
            # Add save to MinIO button
            col1, col2 = st.columns([3, 1])
            with col1:
                filename = st.text_input(
                    "Enter filename for saving to MinIO (optional)",
                    value=csv_file.name if hasattr(csv_file, "name") else None,
                )
            with col2:
                if st.button("Save to MinIO"):
                    try:
                        saved_filename = minio_utils.save_dataframe_to_minio(
                            df, filename
                        )
                        st.success(f"Successfully saved to MinIO as {saved_filename}")
                    except Exception as e:
                        st.error(f"Failed to save to MinIO: {e!s}")
...
def check_train_df(df: pd.DataFrame):
    ...
    if df.shape[1] < 2:
        st.error(
            "Your dataframe has only 1 column. Must include at least 1 Feature and a Target."
        )
        return None, None, False
...
def create_model(data: dict) -> str:
    ...
    response = requests.post(
        f"{API_BASE_URL}/models", json=data, auth=HTTPBasicAuth(USERNAME, PASSWORD)
    )
...
def train_model(model_id: str, model_name: str, data: dict):
    ...
    response = requests.post(
        f"{API_BASE_URL}/models/{model_id}/train",
        json=data,
...
def create_and_train(features: list, targets: list):
    ...
    model_type = st.selectbox(
        "What model will we train?", ["LinearRegression", "RandomForest"]
    )

    model_name = st.text_input("How would you name it?")
    if (models_df is not None) and (
        model_name in models_df["Model Name"].values.tolist()
    ):
        st.warning(
            f"""Model **{model_name}** already exists. Training will override it."""
        )
    ...
    if model_type == "LinearRegression":
        ...
        if n_jobs:
            n_jobs = st.number_input(
                """CPU Cores""", min_value=1, max_value=4, step=1, value=1
            )
        else:
            n_jobs = None
        ...
        max_depth = st.toggle(
            """Max Depth | *The maximum depth of the tree.*""", value=False
        )
        if max_depth:
            max_depth = st.slider(
                """Max Depth""", min_value=100, max_value=1000, step=50, value=100
            )
        else:
            max_depth = None
    ...
    if st.button("Train Model"):
        ...
        with st.spinner(f"""Training ***{create_data["model_name"]}*** model..."""):
            if model_id is None:
                st.error(
                    f"""Error getting ***{create_data["model_name"]}*** model after creation."""
                )
            else:
                train_data = {"features": features, "targets": targets}
                train_model(
                    model_id=model_id,
...
def delete_model(selected_models: list):
    ...
    if st.button("Delete") or st.session_state.delete_model_pending:
        ...
        st.write(
            f"""Following model(s): ***{', '.join(selected_models['Model Name'])}*** will be deleted forever"""
        )
        if st.button("Confirm Delete"):
            for model_id in selected_models["Model ID"].values.tolist():
                ...
                if response.status_code == 200:
                    ...
                    st.error(
                        f"""Error deleting model ***{model_id}***. Response: {response.status_code}."""
                    )
...
def check_predict_test_df(df: pd.DataFrame, target: bool):
    ...
    - Выводит справочную информацию о датасете, чтобы клиент убедился, что данные считаны верно
    - Возвращает отформатированные фичи, таргет (при наличии) и булево значение, отражающее резуль��ат проверки
    """

    if df is None:
        return None, None, False

    if target and df.shape[1] < 2:
        st.error(
            "Your dataframe has only 1 column. Must include at least 1 Feature and a Target."
        )
        st.warning(
            "If you don't have a Target in your data, use the toggle above to pass. You will not be able to test model(s)."
        )
        return None, None, False
    ...
    st.warning(
        "You will be able to predict only as you don't have a Target in your dataset."
    )
...
def predict_model(model_id: str, model_name: str, data: dict) -> pd.DataFrame:
    ...
    response = requests.post(
        f"{API_BASE_URL}/models/{model_id}/predict",
        json=data,
        auth=HTTPBasicAuth(USERNAME, PASSWORD),
    )
    ...
    if response.status_code == 500:
        st.error(
            f"""Model ***{model_name}*** is expecting another number of Features as input."""
        )
        return None
...
def predict(selected_models: list, features: list, df: pd.DataFrame) -> list:
    ...
    if selected_models is not None:
        st.write(
            f"""Selected model(s): ***{', '.join(selected_models["Model Name"])}***"""
        )

        if st.button("Predict"):
            predictions = []
            for id, name in zip(
                selected_models["Model ID"], selected_models["Model Name"]
            ):
    ...
    st.write(
        """*Select model(s) you want to use in the **[Trained models](#trained-models)** section above*"""
    )
    return None
...
def plot_test_scatter(targets: list, predictions: pd.DataFrame):
    ...
    for column in predictions.columns:
        plt.scatter(
            targets, predictions[column], label=column[:-5], alpha=0.25, zorder=3
        )
...
def test(targets: list, predictions: pd.DataFrame):
    ...
    if targets is None:
        ...
        st.error(
            "Error getting Predictions. Please select some [models](#trained-models) and make Predictions with them first."
        )
        ...
        for i, model in enumerate([i[:-5] for i in predictions.columns]):
            test.loc[model, "MAE"] = mean_absolute_error(
                targets, predictions.iloc[:, i]
            )
            test.loc[model, "RMSE"] = np.sqrt(
                mean_squared_error(targets, predictions.iloc[:, i])
            )
        ...
        st.write(
            """*(Random 300 observations were picked to prevent overloading the graph)*"""
        )
...
def view_datasets() -> list:
    """
    Function to view and select datasets from MinIO.

    - Fetches datasets from MinIO
    - Creates an interactive table for dataset selection
    - Returns metadata of selected datasets
    """
    try:
        datasets = minio_utils.list_datasets()
        if not datasets:
            st.warning("No datasets available. Upload one using the form above!")
            return []

        datasets_df = pd.DataFrame(datasets)
        datasets_df["last_modified"] = pd.to_datetime(
            datasets_df["last_modified"]
        ).dt.strftime("(MSK) %Y-%m-%d %H:%M:%S")
        datasets_df["size"] = datasets_df["size"].apply(lambda x: f"{x / 1024:.2f} KB")

        gb = GridOptionsBuilder.from_dataframe(datasets_df)
        gb.configure_selection("single", use_checkbox=True)
        grid_options = gb.build()

        st.write("Select a dataset to use:")
        response = AgGrid(
            datasets_df,
            gridOptions=grid_options,
            enable_enterprise_modules=False,
...
def load_dataset_from_minio(dataset_name: str) -> pd.DataFrame:
    """
    Function to load a dataset from MinIO.

    - Loads the specified dataset from MinIO
    - Converts it to the required format
    - Returns the formatted DataFrame
    """
    try:
        df = minio_utils.get_dataframe_from_minio(dataset_name)
        return df
    except Exception as e:
        st.error(f"Error loading dataset from MinIO: {str(e)}")
        return None


def get_formatted_dataset(dataset_name: str) -> tuple:
    """
    Function to get a formatted dataset ready for the training API.

    - Loads dataset from MinIO
    - Formats it for the training API
    - Returns features and targets
    """
    df = load_dataset_from_minio(dataset_name)
    if df is not None:
        try:
            features, targets = minio_utils.format_dataset_for_training(df)
            ...
            st.write(
                f"Features shape: {len(features)} rows, {len(features[0])} columns"
            )
            st.write(f"Targets shape: {len(targets)} values")

            return features, targets, True
        except Exception as e:
            st.error(f"Error formatting dataset: {str(e)}")
            return None, None, False
    return None, None, False
